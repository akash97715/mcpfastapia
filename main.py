...
async def generate_numbers(request: Request) -> StreamingResponse:
    ...
    return StreamingResponse(number_generator(count), media_type="text/event-stream")
...
async def document_search_generator(query: str, top_k: int) -> AsyncIterator[str]:
    ...
    documents = [
        ...
        },
        {
            ...
            "content": "NLP is a field of AI focused on language understanding...",
        },
        {
            "id": "doc4",
    ...
    for i, doc in enumerate(documents[:top_k]):
        ...
        partial_result = {"type": "partial_result", "result_number": i + 1, "document": result}
...
async def search_documents(request: Request) -> StreamingResponse:
    ...
    return StreamingResponse(
        document_search_generator(query, top_k), media_type="text/event-stream"
    )
